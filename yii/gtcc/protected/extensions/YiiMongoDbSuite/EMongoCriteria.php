			...
			)
			{
				...
				foreach($conds as $operator => $value)
					$this->_conditions[$fieldName][$operator] = $value;
                            ...
                            $splitArray = explode('.', $fieldName);
                            if(count($splitArray) > 1)
                            {
                                // to resolve the search by reference field like 'user.$id'
                                if(!array_key_exists($fieldName, $this->_conditions))
                                {
                                    $this->_conditions[$splitArray[0].".\$".$splitArray[1]] = new MongoId($conds);
                                    unset($conds);
                                }
                            }else{
